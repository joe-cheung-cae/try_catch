...
inline void default_stderr_sink(log_level lvl, const char* file, int line, const char* func, const char* fmt, va_list ap) {
    const char* name = "LOG";
    switch (lvl) {
        ...
        case log_level::off:   name = "OFF";   break;
    }
    std::fprintf(stderr, "[%s] %s:%d %s: ", name,
...
inline void set_log_level(log_level lvl) { runtime_log_level().store(static_cast<int>(lvl), std::memory_order_relaxed); }
inline log_level get_log_level() { return static_cast<log_level>(runtime_log_level().load(std::memory_order_relaxed)); }
inline void set_log_sink(log_sink_t sink) { runtime_sink().store(sink, std::memory_order_relaxed); }
inline log_sink_t get_log_sink() { return runtime_sink().load(std::memory_order_relaxed); }
...
namespace tc { namespace log {
    ...
    inline void set_level(level v) { ::tc::detail::set_log_level(v); }
    inline level get_level() { return ::tc::detail::get_log_level(); }
    using sink_t = ::tc::detail::log_sink_t;
    inline void set_sink(sink_t s) { ::tc::detail::set_log_sink(s); }
    inline sink_t get_sink() { return ::tc::detail::get_log_sink(); }
} }
...
#if TC_EXCEPTIONS_ENABLED
#  define TC_CATCH_STD_WARN()  TC_CATCH(const std::exception&, _tc_e) { TC_WARN("exception: %s", _tc_e.what()); }
#  define TC_CATCH_STD_ERROR() TC_CATCH(const std::exception&, _tc_e) { TC_ERROR("exception: %s", _tc_e.what()); }
#else
#  define TC_CATCH_STD_WARN()  TC_CATCH(const std::exception&, _tc_unused) { TC_WARN("exception handler (no-exceptions build)"); }
#  define TC_CATCH_STD_ERROR() TC_CATCH(const std::exception&, _tc_unused) { TC_ERROR("exception handler (no-exceptions build)"); }
#endif

#define TC_CATCH_ALL_WARN()   TC_CATCH_ALL() { TC_WARN("unknown exception"); }
#define TC_CATCH_ALL_ERROR()  TC_CATCH_ALL() { TC_ERROR("unknown exception"); }
...
#if TC_EXCEPTIONS_ENABLED
#  define TC_CATCH_STD_WARN_DO(BODY)  TC_CATCH(const std::exception&, _tc_e) { TC_WARN("exception: %s", _tc_e.what()); do { BODY; } while(0); }
#  define TC_CATCH_STD_ERROR_DO(BODY) TC_CATCH(const std::exception&, _tc_e) { TC_ERROR("exception: %s", _tc_e.what()); do { BODY; } while(0); }
#  define TC_CATCH_STD_WARN_AS(NAME, BODY)  TC_CATCH(const std::exception&, NAME) { TC_WARN("exception: %s", NAME.what()); do { BODY; } while(0); }
#  define TC_CATCH_STD_ERROR_AS(NAME, BODY) TC_CATCH(const std::exception&, NAME) { TC_ERROR("exception: %s", NAME.what()); do { BODY; } while(0); }
#else
#  define TC_CATCH_STD_WARN_DO(BODY)  TC_CATCH(const std::exception&, _tc_unused) { TC_WARN("exception handler (no-exceptions build)"); do { BODY; } while(0); }
#  define TC_CATCH_STD_ERROR_DO(BODY) TC_CATCH(const std::exception&, _tc_unused) { TC_ERROR("exception handler (no-exceptions build)"); do { BODY; } while(0); }
#  define TC_CATCH_STD_WARN_AS(NAME, BODY)  TC_CATCH(const std::exception&, NAME) { const std::exception& NAME = *static_cast<const std::exception*>(nullptr); TC_WARN("exception handler (no-exceptions build)"); do { BODY; } while(0); }
#  define TC_CATCH_STD_ERROR_AS(NAME, BODY) TC_CATCH(const std::exception&, NAME) { const std::exception& NAME = *static_cast<const std::exception*>(nullptr); TC_ERROR("exception handler (no-exceptions build)"); do { BODY; } while(0); }
#endif

#define TC_CATCH_ALL_WARN_DO(BODY)  TC_CATCH_ALL() { TC_WARN("unknown exception"); do { BODY; } while(0); }
#define TC_CATCH_ALL_ERROR_DO(BODY) TC_CATCH_ALL() { TC_ERROR("unknown exception"); do { BODY; } while(0); }
